...
public class ProprietarioDAO implements IDAO<Proprietario>{
    ...
    public ArrayList<Proprietario> getAll() {
       try(Connection connection = DatabaseHandler.getConnection()){
    	   sql = "SELECT prop.id, prop.nome, prop.cpf, prop.email, prop.id_telefon, tel.numero"
    	   		+ " FROM proprietario prop"
    	   		+ " INNER JOIN telefone tel ON tel.id = prop.id_telefone";
    	   ...
    	   while(result.next()) {
    		   novoProp = new Proprietario(result.getInt("id"), result.getString("nome"), result.getString("cpf"), result.getString("email"), result.getInt("id_telefone"), result.getString("numero"));
    ...
    public Proprietario getById(int id) {
    	 ...
    	 try(Connection connection = DatabaseHandler.getConnection()){
    		  sql = "SELECT prop.id, prop.nome, prop.cpf, prop.email, prop.id_telefone, tel.numero"
    	    	   		+ " FROM proprietario prop"
    	    	   		+ " INNER JOIN telefone tel ON tel.id = prop.id_telefone"
    	    	   		+ " WHERE prop.id = ?";
    		  ...
    		  return new Proprietario(result.getInt("id"), result.getString("nome"), result.getString("cpf"), result.getString("email"), result.getInt("id_telefone"), result.getString("numero"));
